...
## For laptop
df_2020 <- data.frame(rbis)
write.xlsx(df_2020, "c:/users/tdpot/rbis/rbis-per-opportunity/2020_rbis.xlsx")
...
for(i in 1:30){
  for(j in 1:82){
    if(!is.na(home_games_2018[j,i])){
      url <- "https://www.baseball-reference.com/boxes/"
      url <- strcat(url,teams[i])
      url <- strcat(url,"/")
      url <- strcat(url,teams[i])
      url <- strcat(url,"2018")
      url <- strcat(url, home_games_2018[j,i])
      if(j > 1 & strcmp(home_games_2018[j,i],home_games_2018[j-1,i])){
        url <- strcat(url,"2.shtml")
      }
      else if(!is.na(home_games_2018[j+1,i]) & strcmp(home_games_2018[j,i],home_games_2018[j+1,i])){
        url <- strcat(url,"1.shtml")
      }
      else{
        url <- strcat(url,"0.shtml")
...
for(i in 1:30){
  for(j in 1:84){
    if(!is.na(home_games_2017[j,i])){
      url <- "https://www.baseball-reference.com/boxes/"
      url <- strcat(url,teams[i])
      url <- strcat(url,"/")
      url <- strcat(url,teams[i])
      url <- strcat(url,"2017")
      url <- strcat(url, home_games_2017[j,i])
      if(j > 1 & strcmp(home_games_2017[j,i],home_games_2017[j-1,i])){
        url <- strcat(url,"2.shtml")
      }
      else if(!is.na(home_games_2017[j+1,i]) & strcmp(home_games_2017[j,i],home_games_2017[j+1,i])){
        url <- strcat(url,"1.shtml")
      }
      else{
        url <- strcat(url,"0.shtml")
...
for(i in 1:30){
  for(j in 1:81){
    if(!is.na(home_games_2016[j,i])){
      url <- "https://www.baseball-reference.com/boxes/"
      url <- strcat(url,teams[i])
      url <- strcat(url,"/")
      url <- strcat(url,teams[i])
      url <- strcat(url,"2016")
      url <- strcat(url, home_games_2016[j,i])
      if(j > 1 & strcmp(home_games_2016[j,i],home_games_2016[j-1,i])){
        url <- strcat(url,"2.shtml")
      }
      else if(!is.na(home_games_2016[j+1,i]) & strcmp(home_games_2016[j,i],home_games_2016[j+1,i])){
        url <- strcat(url,"1.shtml")
      }
      else{
        url <- strcat(url,"0.shtml")
